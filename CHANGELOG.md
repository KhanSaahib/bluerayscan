...
Seven rounds of the precision programme had all tuned the secrets family in one
direction: downward. Recall was measured only by terragoat and kubernetes-goat,
both infrastructure-as-code, neither of which exercises the 54 documented token
shapes or the two entropy rules. Round eight ran the programme **backwards** --
`Plazmaz/leaky-repo`, a repository built to hold one of every kind of committed
secret, read for every *miss* rather than every finding. It was silent on 48 of
its 61 files.

- **JSON's quoted keys could not be matched at all.** The name's own closing
  quote sits between the name and the colon, so `"HEROKU_API_KEY": "7a2f..."`
  never looked like an assignment. Measured before the fix across 54 saved
  scans of thirty-odd repositories: **3,280 entropy findings, and not one of
  them in a `.json` file**. `appsettings.json`, `serverless.json` and every
  editor's SFTP config are this shape.

  The cost of allowing it is that `cond ? "a" : "b"` reads like an assignment
  too -- bitwarden's `AppHost` and n8n's importer each write one -- so a quoted
  name preceded by a ternary's `?` is skipped.
- **A UUID is generated by definition**, and the entropy floor could never see
  it: hex with four hyphens lands about a sixth of a bit short, every time.
  npm's classic auth token is a UUID and nothing else. The name on the other
  side of the assignment is still the gate, which keeps `client_id` -- the one
  identifier that is usually a UUID and usually public -- out of it.
- **npm scopes a setting by putting the registry inside the key.**
  `//registry.npmjs.org/:_authToken=...`: a non-greedy name stops at the colon
  in the URL long before the `=`, so the name read was `//registry.npmjs.org/`
  and the token went unreported.
- **Seven filenames that mean a credential and mean nothing else**:
  `.git-credentials`, `.htpasswd`, `master.key` (Rails), `.s3cfg`, `.esmtprc`,
  `proftpdpasswd` and `.credentials`. Plus the upload-on-save configs every
  editor grew -- `.ftpconfig`, `sftp-config.json`, `.remote-sync.json` --
  judged on what is in them. `.s3cfg` and `.credentials` are also read as
  value-position formats, which is what an AWS credentials file is.
- **Three false positives the reach exposed**, each fixed rather than
  documented: a word carrying trailing digits is part of a phrase ("Connect
  OAuth2 Credential" -- thirteen of discourse's translated labels were reported
  at medium), a lone dash is a word ("Default - None"), and a path may sit
  under a shouted directory (`GH_CONFIG_DIR/hosts.yml`).
- **`fixture`, singular**, joins the fixture-tree directory names.

Round eight re-measurement: leaky-repo 20 findings to 38, and 13 of its files
to 24. On the pinned corpus, **net medium confidence is −2** -- eleven new (ten
real test-instance passwords in n8n's CI, one JSON example inside a Javadoc
comment) against thirteen removed. The 286 new low-confidence findings are all
in test and fixture trees, 240 of them in bitwarden's vault seeder, which is a
fixture full of generated passwords because it is a fixture for a password
manager. terragoat and kubernetes-goat are unchanged.
