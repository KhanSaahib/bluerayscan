...
_STRUCTURED = (
    ...
    # Authorization Grant", a Keycloak label in eighty locales. A word may also
    # carry trailing digits, which is how a protocol gets into a sentence:
    # n8n's interface says "Connect OAuth2 Credential" and "Custom OAuth2". And
    # a lone dash is a word, because that is how an interface writes an
    # alternative: "Default - None".
    #
    # Digits still may not lead a word in the middle of one:
    # "Bearer eyJhbGciOiJIUzI1NiI..." is also two words with a space, and is a
    # credential.
    re.compile(
        r"^(?:[A-Za-z][A-Za-z'’.,!?-]*\d*|\d+(?:\.\d+)*)"
        r"(?: +(?:[A-Za-z][A-Za-z'’.,!?-]*\d*|\d+(?:\.\d+)*|[-–—]))+:?$"
    ),
    # One word and the punctuation that ends a sentence: "credentials." is what
    # n8n's importer puts in a log line, through a ternary that reads as an
    # assignment once quoted names are allowed. The prose filters above all
    # want a space in the value, and one word has none.
    ...
    # The same path with a shouted directory in front of it, which is how a
    # message names a file relative to somewhere the reader has to look up:
    # gh's tests expect "tokenSource":"GH_CONFIG_DIR/hosts.yml". The first
    # segment has no lower-case in it and every later one has no upper-case,
    # which is a division a base64 run does not make.
    re.compile(r"^[A-Z][A-Z0-9_]*(?:/[a-z0-9._-]+)+$"),
...
#: A canonical UUID, in either case. Version and variant are not checked: a
#: value written in this shape was produced by something that generates them,
#: whichever flavour it used.
...
def entropy_shortfall(value: str) -> "float | None":
    ...
    * counted out -- ``abcdefgh`` measures as maximally random for its length
    ...
    if _UUID.fullmatch(stripped):
        # A UUID is generated by definition, and the entropy floor cannot see
        # that: hex with four hyphens in it lands about a sixth of a bit short,
        # every time. npm's classic auth token is a UUID and nothing else: a
        # line reading "//registry.npmjs.org/:_authToken=" and then one is the
        # whole of the credential, and it was 0.15 bits from being reported.
        # (Written in two pieces on purpose -- GitHub's push protection reads
        # that line as an npm Access Token and refused the commit that first
        # carried it.) The name on the other side of the assignment is still the
        # gate, and "client_id" -- the one identifier that is usually a UUID
        # and usually public -- does not promise a credential.
        return 0.0
